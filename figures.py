...
class Fig3:
    ...
    def check_rotation(self,config,x,y):
        for i in range(0,4):
            print type(x), type(fig3[i][0])
            if x + fig3[config][i][0] < 0 or x + fig3[config][i][0] > n_rows-1 or y + fig3[config][i][1] < 0 or y + fig3[config][i][1] > n_cols-1:
                return 0
            elif self.board.board_matrix[x + fig3[config][i][0]][y + fig3[config][i][1]] == -1 or self.board.board_matrix[x + fig3[config][i][0]][y + fig3[config][i][1]] == -1:
                return 0
        return 1
